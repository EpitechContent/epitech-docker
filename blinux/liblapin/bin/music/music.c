...
int			main(void)
{
  ...
  if ((music = bunny_load_music("hangedbunnysoundlogo.wav")) == NULL)
    return (EXIT_FAILURE);
  ...
  do
	...
	bunny_sound_pitch(music, atof(&buffer[6]));
      else if (strncmp(&buffer[0], "vol=", 4) == 0)
      	bunny_sound_volume(music, atof(&buffer[4]));
      else if (strncmp(&buffer[0], "loop=", 5) == 0)
      	bunny_sound_loop(music, !!atoi(&buffer[5]));
